...
bool search(int hashtable[],int key){
    ...
    if(hashtable[resultant_index]==-1){   
        ...
        int i=0;
        while(hashtable[(resultant_index+i)%10]!=key && hashtable[(resultant_index+i)%10]!=-1){
            i++;
        }
        if(hashtable[(resultant_index+i)%10]==key)
        return true;
    }
    return false;
}
...
int probe(int hashTable[],int key){
    int resultant_index=hash_function(key);
    int i=1;
    while((hashTable[(resultant_index+i)%size_hashtable])!=-1){
        i++;
    }
    return (resultant_index+i)%size_hashtable;
}